...
namespace Evolution.Services.SettingsServices
{
    public static class SettingsService
    {
        ...
        public static void SetRememberDataForAuthSettings(string login, string password, bool isRememberMe)
        {
            ...
            Debug.WriteLine($"\n^^^^^^^^^^^^^^^^^^^^^^^^^\n\t***** ВНИМАНИЕ. Установлена настройка ЗАПОМНИТЬ МЕНЯ {isRememberMe} ******\n");
        }
        ...
        private static void AutoRunState(bool autoRunState = false)
        {
            ...
            if(autoRunState)
            {
                ...
                Debug.WriteLine($"\n^^^^^^^^^^^^^^^^^^^^^^^^^\n\t***** ВНИМАНИЕ. Установлена настройка автозагрузки приложения с запуском системы. ******\n");
            }
            else
            {
                var registerShortcutForAllUser = true;
                ...
                Debug.WriteLine($"\n^^^^^^^^^^^^^^^^^^^^^^^^^\n\t***** ВНИМАНИЕ. Отключена настройка автозагрузки приложения с запуском системы. ******\n");
            }
        }
    }
}
